...
from gym.envs.mujoco import ReacherEnv as ReacherEnv_original
from gym.spaces import Box


class ReacherEnv(ReacherEnv_original):
    """
    ### Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, **kwargs):
        ...
        self.reset_noise_scale_qpos = 0.1
        self.reset_noise_scale_qvel = 0.005

        self.reset_noise_scale_qpos = 0
        self.reset_noise_scale_qvel = 0

        self.init_qpos[:2] += 0.1

    def reset_model(self):
        qpos = (
            self.np_random.uniform(low=-self.reset_noise_scale_qpos, high=self.reset_noise_scale_qpos, size=self.model.nq)
            + self.init_qpos
        )
        while True:
        ...
        qvel = self.init_qvel + self.np_random.uniform(
            low=-self.reset_noise_scale_qvel, high=self.reset_noise_scale_qvel, size=self.model.nv
        )
        qvel[-2:] = 0
    ...
    def step(self, a):
        ...
        theta = self.sim.data.qpos.flat[:2].copy()
        ...
        ob = self._get_obs()
        theta_next = self.sim.data.qpos.flat[:2].copy()
        fingertip = self.get_body_com("fingertip").copy()
        target = self.get_body_com("target")[:-1].copy()
        ...
        return (
            ...
            False,
            False,
            dict(reward_dist=reward_dist, reward_ctrl=reward_ctrl, theta=theta, theta_next=theta_next, fingertip=fingertip, target=target),
        )