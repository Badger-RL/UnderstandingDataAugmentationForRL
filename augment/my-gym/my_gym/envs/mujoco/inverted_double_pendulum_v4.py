...
class InvertedDoublePendulumEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def reset_model(self):
        self.set_state(
            self.init_qpos
            + self.np_random.uniform(low=-self.reset_noise_scale, high=self.reset_noise_scale, size=self.model.nq),
            self.init_qvel + self.np_random.standard_normal(self.model.nv) * self.reset_noise_scale,
        )
