...
class InvertedPendulumEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, reset_noise_scale=0.0, **kwargs):
        utils.EzPickle.__init__(self, **kwargs,)
        observation_space = Box(low=-np.inf, high=np.inf, shape=(4,), dtype=np.float64)
        self.reset_noise_scale = reset_noise_scale

        fullpath = path.join(path.dirname(__file__), "assets", 'inverted_pendulum.xml')
        MujocoEnv.__init__(
            self,
            fullpath,
            2,
    ...
    def reset_model(self):
        qpos = self.init_qpos + self.np_random.uniform(
            size=self.model.nq, low=-self.reset_noise_scale, high=self.reset_noise_scale
        )
        qvel = self.init_qvel + self.np_random.uniform(
            size=self.model.nv, low=-self.reset_noise_scale, high=self.reset_noise_scale
        )
